...
export default function Home(){
 ...
 {melds.length>0&&<div className="meld-row" aria-label="已叫牌">{melds.map((group,i)=><div className="meld-group" key={i}><span>{group.length===4?'槓':'碰'}</span>{group.map(tile=><TileFace key={tile.id} tile={tile} small/>)}</div>)}</div>}<div className={`hand-row ${opening?'dealing-hand':''}`} ref={handRow} aria-busy={busy}>{hand.map((tile,i)=><TileFace key={tile.id} tile={tile} active={!busy&&(selectedTile===tile.id||(!selectedTile&&hints&&i===bestDiscard))} drawn={tile.id===drawnId} onClick={!opening&&!busy&&!table.result&&!ownDecision&&hand.length===14-melds.length*3?()=>setSelectedTile(tile.id):undefined}/>)}</div><div className="action-row"><span>{table.result?'本局已完結':claimPending||ownDecision?'等你決定叫牌或過':busy?'等候牌友出牌…':selectedTile?'已選牌，確認後打出':'點牌選取，再按出牌'}</span><div>{selectedTile&&hand.find(t=>t.id===selectedTile)&&<span className="selected-preview"><MahjongFace tile={hand.find(t=>t.id===selectedTile)!}/></span>}<Button className="discard-button" disabled={busy||!!opening||!!table.result||ownDecision||!hand.some(t=>t.id===selectedTile)} onClick={()=>{const index=hand.findIndex(t=>t.id===selectedTile);if(index>=0)void discard(index);}}>出牌</Button></div></div></div></section>}
