...
export default function Home(){
 ...
 function react(ai:number,event:'think'|'meld'|'win'|'lose',detail=''){const lines={think:['諗諗先… 🤔','呢隻好唔好呢？ 💭','要小心啲… 😅'],meld:['有得叫！ 😏','啱晒！ 😄','等陣先！ ✋'],win:['食糊！承讓承讓！ 😄','終於等到喇！ 🎉','今日幾好手氣喎！ 😎'],lose:['哎吔，放銃添！ 😓','差少少就到我喇… 😩','下舖追返！ 💪']};setReactions(r=>({...r,[ai]:detail||lines[event][Math.floor(Math.random()*lines[event].length)]}));}
 ...
 useEffect(()=>{const board=tableElement.current;if(!board)return;
  const fit=()=>{const rect=board.getBoundingClientRect(),border=board.clientTop;const bounds=(selector:string)=>board.querySelector(selector)?.getBoundingClientRect();
   ...
   const rows=Math.max(1,Math.ceil(discarded.length/12)),columns=Math.min(12,Math.max(1,discarded.length));
 ...
 useEffect(()=>{if(ready&&resume.current){resume.current=false;void runOpponents(hand,[...wall],[...discarded],aiHands.map(x=>[...x]),0,flow??undefined);}},[ready]);
 ...
 function endDraw(){setFlow(null);setLastPlay(null);setActiveAI(null);setBusy(false);locked.current=true;setTable(t=>({...t,result:{winner:null,selfDrawn:false,score:{fan:0,patterns:[]},changes:[0,0,0,0],description:'流局 · 無牌可補'}}));}
 ...
 async function runOpponents(nextHand:Tile[],nextWall:Tile[],river:Tile[],updated:Tile[][],firstAI:number,continuation?:Flow,skipHuman=false){
  ...
  while(sequence.current===run){
   ...
   if(cursor.phase==='rob'){
    ...
    const winners=Array.from({length:3},(_,i)=>(player+i+1)%4).map(p=>({player:p,score:scoreWithFlowers([...(p===0?nextHand:updated[p-1]),tile],p===0?melds:exposed[p-1],false,p===0?seat:opponents[p-1].seat,flowersRef.current[p])})).filter(w=>eligible(w.score,table.chicken)&&!(w.player===0&&skipHuman));
   ...
   if(cursor.phase==='claims'){
    ...
    const wins=Array.from({length:3},(_,i)=>(source+i+1)%4).map(player=>({player,score:scoreWithFlowers([...(player===0?nextHand:updated[player-1]),drop],player===0?melds:exposed[player-1],false,player===0?seat:opponents[player-1].seat,flowersRef.current[player])})).filter(w=>eligible(w.score,table.chicken)&&!(w.player===0&&skipHuman));
 ...
 function flowerRack(player:number){
  const list=flowers[player],name=player===0?'你':table.names[player-1],wind=player===0?seat:opponents[player-1].seat;
  if(opening||!list.length)return null;
  return <Dialog><DialogTrigger render={<button className="seat-flowers" aria-label={`放大查看${name}的花牌`}><span className="seat-flower-label">{name} · 花 {list.length}</span><span className="seat-flower-tiles">{list.map(tile=><span className="flower-mini" key={tile.id}><MahjongFace tile={tile}/></span>)}</span><span className="flower-enlarge">點按放大</span></button>}/><DialogContent className="flower-detail"><DialogHeader><DialogTitle>{name}的花牌 · {wind}位</DialogTitle></DialogHeader><p>摸花後已從牌尾補牌。正花結算時每張加一番。</p><div className="flower-detail-tiles">{list.map(tile=><figure key={tile.id}><TileFace tile={tile}/><figcaption>{FLOWER_NAMES[tile.flower!]}{tile.flower!%4==='東南西北'.indexOf(wind)?' · 正花':''}</figcaption></figure>)}</div></DialogContent></Dialog>;
 }
 ...
 {view==='play'&&started&&<section className="game-page"><div className="game-status"><div><span className="round-badge">東一局</span><b>第 {turn} 巡</b><span>餘 {wall.length} 張</span></div><div className="scores"><span><i>{seat}</i>你 <b>{money(table.balances[0])}</b></span>{opponents.map(p=><span key={p.name}><i>{p.seat}</i>{p.name}<b>{money(p.score)}</b></span>)}</div><div className="save-actions"><Button variant="outline" onClick={()=>openConfirmation('save')}>{savedNotice?'已儲存':'儲存'}</Button><Button variant="outline" onClick={()=>openConfirmation('leave')}><RotateCcw/>離枱</Button></div></div><div className="mobile-balance"><button className="sound-toggle" aria-label={sound?'關閉聲效':'開啟聲效'} aria-pressed={sound} onClick={()=>toggleSound(!sound)}>{sound?'🔊 聲效開':'🔇 聲效關'}</button>本錢 {money(table.balances[0])} · 底 {money(table.baseCents)} · {table.chicken?'零番起糊':'三番起糊'}</div><div className="table-wrap"><div className="mahjong-table" ref={tableElement}>{opening&&<OpeningSequence opening={opening} names={['你',...table.names]} onRoll={()=>setOpening(o=>o?{...o,index:o.index+1}:null)} onSkip={()=>{stopSounds();setOpening(null);}}/>}<div className={`opponent north ${activeAI===1?'opponent-active':''}`}><Avatar value={opponents[1].avatar}/><span>{opponents[1].name}<small>{aiHands[1].length} 張</small></span>{reactions[1]&&<span className="ai-bubble" role="status">{reactions[1]}</span>}<div className="seat-public">{aiMelds[1].length>0&&<div className="ai-melds">{aiMelds[1].map((m,i)=><span key={i} title={m.length===4?'槓':isChow(m)?'上':'碰'}>{m.map(t=><TileFace key={t.id} tile={t} small/>)}</span>)}</div>}{flowerRack(2)}</div></div><div className={`opponent west ${activeAI===2?'opponent-active':''}`}><Avatar value={opponents[2].avatar}/><span>{opponents[2].name}<small>{aiHands[2].length} 張</small></span>{reactions[2]&&<span className="ai-bubble" role="status">{reactions[2]}</span>}<div className="seat-public">{aiMelds[2].length>0&&<div className="ai-melds">{aiMelds[2].map((m,i)=><span key={i} title={m.length===4?'槓':isChow(m)?'上':'碰'}>{m.map(t=><TileFace key={t.id} tile={t} small/>)}</span>)}</div>}{flowerRack(3)}</div></div><div className={`opponent east ${activeAI===0?'opponent-active':''}`}><Avatar value={opponents[0].avatar}/><span>{opponents[0].name}<small>{aiHands[0].length} 張</small></span>{reactions[0]&&<span className="ai-bubble" role="status">{reactions[0]}</span>}<div className="seat-public">{aiMelds[0].length>0&&<div className="ai-melds">{aiMelds[0].map((m,i)=><span key={i} title={m.length===4?'槓':isChow(m)?'上':'碰'}>{m.map(t=><TileFace key={t.id} tile={t} small/>)}</span>)}</div>}{flowerRack(1)}</div></div><div className="wall wall-north" role="img" aria-label={`${opponents[1].name}：${aiHands[1].length} 張暗牌`}>{Array.from({length:aiHands[1].length}).map((_,i)=><i key={i}/>)}</div><div className="wall wall-west" role="img" aria-label={`${opponents[2].name}：${aiHands[2].length} 張暗牌`}>{Array.from({length:aiHands[2].length}).map((_,i)=><i key={i}/>)}</div><div className="wall wall-east" role="img" aria-label={`${opponents[0].name}：${aiHands[0].length} 張暗牌`}>{Array.from({length:aiHands[0].length}).map((_,i)=><i key={i}/>)}</div><div className="table-center"><span>東</span><b>一</b><small>圈風 東</small></div>{lastPlay&&<div key={lastPlay.tile.id} className={`last-play from-${['east','north','west'][lastPlay.ai]}`}><span>{opponents[lastPlay.ai].name} 出牌</span><TileFace tile={lastPlay.tile}/></div>}{discarded.length>0&&<Dialog><DialogTrigger render={<button className="discard-pool river-grid" style={{top:riverFit.top,['--fitted-river-tile' as string]:`${riverFit.tile}px`}} aria-label={`放大查看牌河，共 ${discarded.length} 張`}><span className="river-caption">牌河 · {discarded.length} 張 · 點按放大</span><span className="river-tiles" style={{['--river-columns' as string]:Math.min(12,discarded.length),['--river-row-count' as string]:Math.ceil(discarded.length/12),['--river-rows' as string]:Math.max(4,Math.ceil(discarded.length/12)),['--river-count' as string]:discarded.length}}>{discarded.map(t=><span key={t.id} className={`river-tile ${lastPlay?.tile.id===t.id?'river-latest':''}`}><MahjongFace tile={t}/></span>)}</span></button>}/><DialogContent className="river-dialog"><DialogHeader><DialogTitle>牌河 · {discarded.length} 張</DialogTitle></DialogHeader><p>按出牌先後排列，金框係最新打出的牌。已被上、碰、槓的牌會移到叫牌者面前。</p><div className="river-detail">{discarded.map((t,i)=><figure key={t.id}><TileFace tile={t} small/><figcaption>{i+1}</figcaption></figure>)}</div></DialogContent></Dialog>}</div><Dialog><DialogTrigger render={<Button variant="outline" className="coach-toggle"><Lightbulb/><span>牌友提示</span></Button>}/><DialogContent className="coach-dialog"><DialogHeader><DialogTitle>牌友提示</DialogTitle></DialogHeader><div className="coach-content"><div className="coach-head"><Lightbulb/><span><b>牌友提示</b><small>{difficulty}分析</small></span></div>{hints?<><p>先打 <b>{bestDiscard>=0?hand[bestDiscard]?.honor:'孤張字牌'}</b>，保留兩面搭子，向平糊方向行。</p><div className="ukeire"><span>AI 讀局</span><p>{aiRead}</p><small>只分析公開資訊，不會偷睇手牌</small></div></>:<p>提示已關閉。靠自己眼光打呢舖！</p>}<Dialog><DialogTrigger render={<Button variant="outline" className="fan-button"><Calculator/>番數計算器</Button>}/><DialogContent><DialogHeader><DialogTitle>番數計算器</DialogTitle></DialogHeader><FanCalculator fan={fan} setFan={setFan} total={fanTotal}/></DialogContent></Dialog></div></DialogContent></Dialog></div><div className="hand-dock">{!opening&&flowerNotice&&flowerNotice.player===0&&<div className="flower-notice" role="status">{flowerNotice.player===0?'你':table.names[flowerNotice.player-1]} 摸花：{flowerNotice.tiles.map(tile=><TileFace key={tile.id} tile={tile} small/>)}<span>已從牌尾補牌</span></div>}{flowerRack(0)}<div className="turn-message" role="status" aria-live="polite"><span className="pulse"/>{opening?'開局準備中…':message}</div>{claimPending&&<div className="claim-choice" role="group" aria-label="叫牌決定"><strong>等你決定</strong>{eligible(pendingScore,table.chicken)&&<Button className="claim-button" onClick={()=>decideClaim('win')}>食糊 · {pendingScore!.fan} 番</Button>}{!competingWin&&chows.map((tiles,i)=><Button key={i} className="claim-button" onClick={()=>decideClaim('chow',i)}>上 {sortHand([...tiles,lastPlay!.tile]).map(t=>t.n).join('')} {tiles[0].suit}</Button>)}{!robbing&&!competingWin&&matches.length>=3&&wall.length>0&&<Button className="claim-button" onClick={()=>decideClaim('kong')}>槓</Button>}{!robbing&&!competingWin&&matches.length>=2&&melds.length<4&&<Button className="claim-button" onClick={()=>decideClaim('pung')}>碰</Button>}<Button variant="outline" onClick={()=>decideClaim('pass')}>過</Button></div>}
