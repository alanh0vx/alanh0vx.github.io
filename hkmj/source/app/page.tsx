...
import { defaultTable, eligible, kongChoices, pickNames, scoreWithFlowers, settle, type HandScore } from '@/lib/mahjong/table-rules';
...
export default function Home(){
 const [table,setTable]=useState(()=>({...defaultTable(),names:pickNames()}));
 ...
 const opponents=table.names.map((name,i)=>({name,avatar:['🦊','🐲','🦁'][i],seat:'東南西北'[('東南西北'.indexOf(seat==='隨機'?'東':seat)+i+1)%4],score:table.balances[i+1]}));
 ...
 useEffect(()=>{
  try {const saved=parseSave(localStorage.getItem(SAVE_KEY));if(saved){
   setFlowers(saved.flowers);flowersRef.current=saved.flowers;setOpening(saved.opening);setTable(saved.table);setMelds(saved.melds);setClaimPending(saved.claimPending);setStarted(saved.started);setWall(saved.wall);setHand(saved.hand);setAiHands(saved.aiHands);setDiscarded(saved.discarded);setTurn(saved.turn);setDrawnId(saved.drawnId);setBusy(saved.busy);setActiveAI(saved.activeAI);setLastPlay(saved.lastPlay);
   setDifficulty(saved.difficulty);setRounds(saved.rounds);setSeat(saved.seat);setHints(saved.hints);setSound(saved.sound);setAvatar(saved.avatar);setMessage(saved.message);setAiRead(saved.aiRead);resume.current=saved.started&&saved.busy&&!saved.claimPending&&!saved.table.result&&!saved.opening;
  }}catch{setSaveError(true);}setReady(true);
 },[]);
 useEffect(()=>{
  if(!ready||preserveSave.current)return;
  saveCurrent();
 },[ready,flowers,opening,table,melds,claimPending,started,wall,hand,aiHands,discarded,turn,drawnId,busy,activeAI,lastPlay,difficulty,rounds,seat,hints,sound,avatar,message,aiRead]);
 useEffect(()=>{if(ready&&resume.current){resume.current=false;void runOpponents(hand,[...wall],[...discarded],aiHands.map(x=>[...x]),activeAI===null?0:lastPlay?activeAI+1:activeAI);}},[ready]);
 function saveCurrent(){
  try {localStorage.setItem(SAVE_KEY,JSON.stringify({version:1,flowers,opening,table,melds,claimPending,started,wall,hand,aiHands,discarded,turn,drawnId,busy,activeAI,lastPlay,difficulty,rounds,seat,hints,sound,avatar,message,aiRead}));setSaveError(false);return true;}catch{setSaveError(true);return false;}
 }
 ...
 function endDraw(){setTable(t=>({...t,result:{winner:null,selfDrawn:false,score:{fan:0,patterns:[]},changes:[0,0,0,0],description:'流局 · 無牌可補'}}));}
 ...
 function startGame(continuing=false){setTable(t=>({...t,result:null,ownPassed:false,balances:continuing?t.balances:Array(4).fill(t.initialCents),names:t.names}));if(seat==='隨機')setSeat('東南西北'[Math.floor(Math.random()*4)]);setMelds([]);setClaimPending(false);claimLock.current=false;preserveSave.current=false;setSavedNotice(false);sequence.current++;locked.current=false;setBusy(false);setActiveAI(null);setLastPlay(null);setDrawnId(null);if(sound)unlockSound();const prepared=prepareOpening(),mine=prepared.hands[0],bots=prepared.hands.slice(1);setOpening(prepared.opening);setFlowers(prepared.flowers);flowersRef.current=prepared.flowers;setFlowerNotice(null);setHand([...sortHand(mine.slice(0,13)),mine[13]]);setDrawnId(mine[13].id);setAiHands(bots);setWall(prepared.wall);setDiscarded([]);setTurn(1);setStarted(true);setMessage('輪到你出牌');setAiRead(`${difficulty} AI 會按牌效、巡目同牌河決策`);setView('play')}
 async function discard(index:number){
  ...
  setFlowerNotice(null);locked.current=true;setTable(t=>({...t,ownPassed:false}));setSavedNotice(false);setBusy(true);setDrawnId(null);setLastPlay(null);
 ...
 async function runOpponents(nextHand:Tile[],nextWall:Tile[],river:Tile[],updated:Tile[][],firstAI:number){
  ...
  if(river.length&&(firstAI===3||updated[firstAI].length===13)){
   const otherWin=opponentWin(updated,river.at(-1)!,firstAI);
   if(otherWin){finishWin(otherWin.player,firstAI,otherWin.score);return;}
  }
  for(let ai=firstAI;ai<3;ai++){
   if(updated[ai].length===13){const draw=takeDraw(ai+1,nextWall);if(!draw)break;updated[ai].push(draw);}
   setWall([...nextWall]);setAiHands(updated.map(x=>[...x]));setActiveAI(ai);setLastPlay(null);setMessage(`${opponents[ai].name} 摸牌，諗緊出邊隻…`);
   if(!await pause(650))return;
   const aiScore=scoreWithFlowers(updated[ai],[],true,opponents[ai].seat,flowersRef.current[ai+1]);
   if(eligible(aiScore,table.chicken)){finishWin(ai+1,null,aiScore!);return;}
   const idx=chooseAIDiscard(updated[ai],river,difficulty,turn,opponents[ai].score<table.balances[0]),drop=updated[ai].splice(idx,1)[0];
   river.push(drop);setDiscarded([...river]);setAiHands(updated.map(x=>[...x]));setLastPlay({tile:drop,ai});setMessage(`${opponents[ai].name} 打出 ${drop.n??''}${drop.suit??drop.honor}`);
   if(sound)playSound('tile');
   const otherWin=opponentWin(updated,drop,ai+1),playerCanWin=eligible(scoreWithFlowers([...nextHand,drop],melds,false,seat,flowersRef.current[0]),table.chicken);
   if(otherWin&&(!playerCanWin||(otherWin.player-(ai+1)+4)%4<(4-(ai+1))%4)){finishWin(otherWin.player,ai+1,otherWin.score);return;}
   if((melds.length<4&&nextHand.filter(t=>tileKey(t)===tileKey(drop)).length>=2)||eligible(scoreWithFlowers([...nextHand,drop],melds,false,seat,flowersRef.current[0]),table.chicken)){
    setClaimPending(true);claimLock.current=false;setMessage(`${opponents[ai].name} 打出 ${drop.n??''}${drop.suit??drop.honor}，請決定叫牌或過`);return;
   }
   if(!await pause(850))return;
  }
  const draw=takeDraw(0,nextWall);setTable(t=>({...t,ownPassed:false}));setActiveAI(null);setLastPlay(null);setHand(draw?[...nextHand,draw]:nextHand);setDrawnId(draw?.id??null);setWall(nextWall);setTurn(t=>t+1);
  setAiRead(turn>7?'AI 轉入守勢，優先打熟張':'AI 保留搭子，改善入張');setMessage(draw?`摸到 ${draw.n??''}${draw.suit??draw.honor}，右邊新牌，請出牌`:'牌局完結');
  locked.current=false;setBusy(false);if(!draw)setTable(t=>({...t,result:{winner:null,selfDrawn:false,score:{fan:0,patterns:[]},changes:[0,0,0,0],description:'流局 · 無人食糊'}}));if(sound&&draw)playSound('draw');
 }

 function opponentWin(cards:Tile[][],tile:Tile,discarder:number){
  for(let step=1;step<4;step++){const player=(discarder+step)%4;if(player===0)continue;const score=scoreWithFlowers([...cards[player-1],tile],[],false,opponents[player-1].seat,flowersRef.current[player]);if(eligible(score,table.chicken))return {player,score:score!};}return null;
 }
 const competingWin=lastPlay?opponentWin(aiHands,lastPlay.tile,lastPlay.ai+1):null;
 const pendingScore=lastPlay?scoreWithFlowers([...hand,lastPlay.tile],melds,false,seat,flowers[0]):null;
 const ownScore=scoreWithFlowers(hand,melds,!!drawnId,seat,flowers[0]);
 ...
 function finishWin(winner:number,discarder:number|null,score:HandScore){
  if(sound)playSound('win');sequence.current++;locked.current=true;setBusy(false);setClaimPending(false);setActiveAI(null);setLastPlay(null);setSavedNotice(false);
  const changes=settle(winner,discarder,score.fan,table.baseCents),description=`${winner===0?'你':opponents[winner-1].name}${discarder===null?'自摸':'食糊'} · ${score.fan} 番`;
 ...
 function decideClaim(kind:'pung'|'kong'|'win'|'pass'){
  ...
  if((kind==='pung'||kind==='kong')&&competingWin)return;
  claimLock.current=true;setClaimPending(false);setSavedNotice(false);
  if(kind==='pass'){void runOpponents([...hand],[...wall],[...discarded],aiHands.map(x=>[...x]),lastPlay.ai+1);return;}
  if(kind==='win'){finishWin(0,lastPlay.ai+1,pendingScore!);return;}
  const taken=matches.slice(0,kind==='kong'?3:2),ids=new Set(taken.map(t=>t.id)),nextHand=sortHand(hand.filter(t=>!ids.has(t.id))),nextWall=[...wall];
  const replacement=kind==='kong'?takeDraw(0,nextWall,true):undefined;
  sequence.current++;setMelds([...melds,[...taken,lastPlay.tile]]);setHand(replacement?[...nextHand,replacement]:nextHand);setWall(nextWall);setDiscarded(discarded.slice(0,-1));
  setTable(t=>({...t,ownPassed:false}));setLastPlay(null);setActiveAI(null);setDrawnId(replacement?.id??null);setBusy(false);locked.current=false;setMessage(kind==='kong'?'槓！已補牌，請出牌。':'碰！請揀一隻牌打出，唔使摸牌。');if(kind==='kong'&&!replacement)endDraw();if(sound)playSound(kind==='kong'?'kong':'pung');
 }
 function declareKong(id:string){
  ...
  if(choice.meldIndex>=0){for(let ai=0;ai<3;ai++){const score=scoreWithFlowers([...aiHands[ai],choice.tile],[],false,opponents[ai].seat,flowers[ai+1]);if(eligible(score,table.chicken)){finishWin(ai+1,0,score!);return;}}}
 ...
 <details className="table-rules"><summary>本枱番數及收付表</summary><p>本枱採用全辣、八番封頂。每番翻倍；出銃者付雙份，其餘兩家各一份；自摸三家各付雙份。槓不另收錢。本錢可記負數。一炮一響，依出牌後的順序決定食糊優先；食糊優先於碰槓。</p><table><thead><tr><th>番</th><th>出銃</th><th>其餘各付</th><th>自摸各付</th></tr></thead><tbody>{[0,1,2,3,4,5,6,7,8].map(f=><tr key={f}><td>{f===0?'雞糊':f}</td><td>{money(table.baseCents*2**f*2)}</td><td>{money(table.baseCents*2**f)}</td><td>{money(table.baseCents*2**f*2)}</td></tr>)}</tbody></table><p>計番：自摸、平糊、番牌及風刻各 1；混一色、對對糊、混么九 3；小三元 5；清一色 7；字一色、清么九、大三元、小四喜 10；大四喜、十三么 13。以最高合法組合計番。共 144 張，含梅蘭菊竹、春夏秋冬。花牌攤開，從牌尾補牌；正花每張加一番（只計結算，不當起糊番數）。無花、門清及花牌組合不另加番。</p></details>
 <div className="switch-line"><span><Lightbulb/> 出牌提示<small>顯示牌效較高嘅選擇</small></span><Switch checked={hints} onCheckedChange={setHints}/></div><div className="switch-line sound-line"><span>🔊 麻雀聲效<small>洗牌、疊牌、骰仔、補花及叫牌聲</small></span><Switch aria-label="麻雀聲效" checked={sound} onCheckedChange={toggleSound}/></div><Button className="shuffle-opponents" variant="outline" onClick={()=>setTable(t=>({...t,names:pickNames()}))}>換一批牌友</Button><div className="players-preview"><div><Avatar value={avatar}/><span>你<small>{seat==='隨機'?'隨機位':`${seat}位`}</small></span></div>{opponents.map(p=><div key={p.name}><Avatar value={p.avatar}/><span>{p.name}<small>{difficulty}</small></span></div>)}</div>{preserveSave.current&&<Button className="resume-button" variant="outline" onClick={()=>window.location.reload()}>繼續已儲存牌局</Button>}<Button className="start-button" onClick={()=>startGame()}><Play/>{preserveSave.current?'開新局':'埋位開枱'}</Button></div></section>}
 {view==='play'&&started&&<section className="game-page"><div className="game-status"><div><span className="round-badge">東一局</span><b>第 {turn} 巡</b><span>餘 {wall.length} 張</span></div><div className="scores"><span><i>{seat}</i>你 <b>{money(table.balances[0])}</b></span>{opponents.map(p=><span key={p.name}><i>{p.seat}</i>{p.name}<b>{money(p.score)}</b></span>)}</div><div className="save-actions"><Button variant="outline" onClick={()=>openConfirmation('save')}>{savedNotice?'已儲存':'儲存'}</Button><Button variant="outline" onClick={()=>openConfirmation('leave')}><RotateCcw/>離枱</Button></div></div><div className="mobile-balance"><button className="sound-toggle" aria-label={sound?'關閉聲效':'開啟聲效'} aria-pressed={sound} onClick={()=>toggleSound(!sound)}>{sound?'🔊 聲效開':'🔇 聲效關'}</button>本錢 {money(table.balances[0])} · 底 {money(table.baseCents)} · {table.chicken?'零番起糊':'三番起糊'}</div><div className="table-wrap"><div className="mahjong-table">{opening&&<OpeningSequence opening={opening} names={['你',...table.names]} onRoll={()=>setOpening(o=>o?{...o,index:o.index+1}:null)} onSkip={()=>{stopSounds();setOpening(null);}}/>}<div className={`opponent north ${activeAI===1?'opponent-active':''}`}><Avatar value={opponents[1].avatar}/><span>{opponents[1].name}<small>{aiHands[1].length} 張</small></span></div><div className={`opponent west ${activeAI===2?'opponent-active':''}`}><Avatar value={opponents[2].avatar}/><span>{opponents[2].name}<small>{aiHands[2].length} 張</small></span></div><div className={`opponent east ${activeAI===0?'opponent-active':''}`}><Avatar value={opponents[0].avatar}/><span>{opponents[0].name}<small>{aiHands[0].length} 張</small></span></div><div className="wall wall-north">{Array.from({length:14}).map((_,i)=><i key={i}/>)}</div><div className="wall wall-west">{Array.from({length:9}).map((_,i)=><i key={i}/>)}</div><div className="wall wall-east">{Array.from({length:9}).map((_,i)=><i key={i}/>)}</div><div className="table-center"><span>東</span><b>一</b><small>圈風 東</small></div>{lastPlay&&<div key={lastPlay.tile.id} className={`last-play from-${['east','north','west'][lastPlay.ai]}`}><span>{opponents[lastPlay.ai].name} 出牌</span><TileFace tile={lastPlay.tile}/></div>}<div className="discard-pool">{discarded.slice(-24).map((t,i)=><TileFace key={`${t.id}-${i}`} tile={t} active={lastPlay?.tile.id===t.id} small/>)}</div></div><aside className="coach"><div className="coach-head"><Lightbulb/><span><b>牌友提示</b><small>{difficulty}分析</small></span></div>{hints?<><p>先打 <b>{bestDiscard>=0?hand[bestDiscard]?.honor:'孤張字牌'}</b>，保留兩面搭子，向平糊方向行。</p><div className="ukeire"><span>AI 讀局</span><p>{aiRead}</p><small>只分析公開資訊，不會偷睇手牌</small></div></>:<p>提示已關閉。靠自己眼光打呢舖！</p>}<Dialog><DialogTrigger render={<Button variant="outline" className="fan-button"><Calculator/>番數計算器</Button>}/><DialogContent><DialogHeader><DialogTitle>番數計算器</DialogTitle></DialogHeader><FanCalculator fan={fan} setFan={setFan} total={fanTotal}/></DialogContent></Dialog></aside></div><div className="hand-dock">{!opening&&flowerNotice&&<div className="flower-notice" role="status">{flowerNotice.player===0?'你':table.names[flowerNotice.player-1]} 摸花：{flowerNotice.tiles.map(tile=><TileFace key={tile.id} tile={tile} small/>)}<span>已從牌尾補牌</span></div>}{!opening&&flowers.some(list=>list.length>0)&&<div className="flower-racks" aria-label="各家花牌">{flowers.map((list,i)=>list.length>0&&<div key={i}><span>{i===0?'你':table.names[i-1]} · 花</span>{list.map(tile=><TileFace key={tile.id} tile={tile} small/>)}</div>)}</div>}<div className="turn-message" role="status" aria-live="polite"><span className="pulse"/>{opening?'開局準備中…':message}</div>{claimPending&&<div className="claim-choice" role="group" aria-label="叫牌決定"><strong>等你決定</strong>{eligible(pendingScore,table.chicken)&&<Button className="claim-button" onClick={()=>decideClaim('win')}>食糊 · {pendingScore!.fan} 番</Button>}{!competingWin&&matches.length>=3&&wall.length>0&&<Button className="claim-button" onClick={()=>decideClaim('kong')}>槓</Button>}{!competingWin&&matches.length>=2&&melds.length<4&&<Button className="claim-button" onClick={()=>decideClaim('pung')}>碰</Button>}<Button variant="outline" onClick={()=>decideClaim('pass')}>過</Button></div>}
 ...
 {table.result&&<div className="hand-result" role="status"><h2>{table.result.description}</h2><p>{table.result.score.patterns.map(p=>`${p.name} ${p.fan}番`).join(' · ')}</p><div className="result-balances">{['你',...table.names].map((name,i)=><div key={name}><b>{name}</b><span>{table.result!.changes[i]>=0?'+':''}{money(table.result!.changes[i])}</span><strong>餘額 {money(table.balances[i])}</strong></div>)}</div><Button className="claim-button" onClick={()=>startGame(true)}>下一局 · 保留本錢</Button></div>}
 {melds.length>0&&<div className="meld-row" aria-label="已叫牌">{melds.map((group,i)=><div className="meld-group" key={i}><span>{group.length===4?'槓':'碰'}</span>{group.map(tile=><TileFace key={tile.id} tile={tile} small/>)}</div>)}</div>}<div className={`hand-row ${opening?'dealing-hand':''}`} ref={handRow} aria-busy={busy}>{hand.map((tile,i)=><TileFace key={tile.id} tile={tile} active={!busy&&hints&&i===bestDiscard} drawn={tile.id===drawnId} onClick={!opening&&!busy&&!table.result&&!ownDecision&&hand.length===14-melds.length*3?()=>void discard(i):undefined}/>)}</div><div className="action-row"><span>{table.result?'本局已完結':claimPending||ownDecision?'等你決定叫牌或過':busy?'等候牌友出牌…':drawnId?'右邊金框係新摸牌 · 點擊出牌':'手牌已自動排序 · 點擊出牌'}</span><div><Button variant="outline" disabled>碰</Button><Button variant="outline" disabled>槓</Button><Button disabled>食</Button></div></div></div></section>}
