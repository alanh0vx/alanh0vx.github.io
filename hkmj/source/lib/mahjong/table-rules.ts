...
export function scoreHand(hand:Tile[],melds:Tile[][]=[],selfDrawn=false,seat='東',round='東'):HandScore|null {
 ...
 const exposed=melds.filter(m=>!isChow(m)).map(m=>tileIndex(m[0]));
 ...
 for(let i=0;i<34;i++)if(counts[i]>=2){counts[i]-=2;groups(i,[],melds.filter(isChow).length);counts[i]+=2;}
 return best;
}
...
export function kongChoices(hand:Tile[],melds:Tile[][],wallSize:number){
 ...
 const meldIndex=melds.findIndex(m=>m.length===3&&!isChow(m)&&tileIndex(m[0])===tileIndex(tile));
