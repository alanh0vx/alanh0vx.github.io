...
export function parseSave(raw: string|null): GameSave|null {
 try {
  ...
  if(!s.melds.every(m=>Array.isArray(m)&&[3,4].includes(m.length)&&m.every(t=>t.suit===m[0].suit&&t.n===m[0].n&&t.honor===m[0].honor)))return null;
  ...
  const groups=[s.wall,s.hand,s.discarded,...s.aiHands,...s.melds,...s.flowers];if(!groups.every(Array.isArray))return null;
  if(s.started){
   ...
   if(s.claimPending){
    ...
    if(!canPung&&!eligible(scoreHand([...s.hand,s.lastPlay.tile],s.melds,false,s.seat),t.chicken))return null;
   }
   if(!s.aiHands.every((h,i)=>h.length===((s.busy&&s.activeAI===i&&!s.lastPlay)||(t.result?.winner===i+1&&t.result.selfDrawn)?14:13)))return null;
   if(!s.busy&&(s.activeAI!==null||s.lastPlay!==null))return null;
   if(s.lastPlay&&(s.lastPlay.ai!==s.activeAI||s.lastPlay.tile.id!==s.discarded.at(-1)?.id))return null;
